...
<link href="{{ asset('css/Admin/media_videos.css') }}" rel="stylesheet" />
        ...
        <table class="table table-hover table-custom" id="document-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <video autoplay muted loop src="{{ asset('sales_manager/media_database/videos/' . $Video->name) }}" alt="{{ $Video->name }}" class="img-preview">
                    </td>
                    <td>{{ $Video->name }}</td>
                    <td class="text-center">{{ $Video->created_at->format('d M Y') }}</td>
                    <td>
                        <div class="action-buttons">
                            <a href="{{ asset('sales_manager/media_database/videos/' . $Video->name) }}" target="_blank" class="action-button" title="View">
                            ...
                            <a href="{{ asset('sales_manager/media_database/videos/' . $Video->name) }}" class="action-button" download title="Download">
                            ...
                            <form action="{{ route('MediaVideos.destroy', $Video->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="action-button delete-btn" title="Delete" data-document-id="{{ $Video->id }}">
...
<div class="modal fade" id="fileUploadModal" tabindex="-1" aria-labelledby="fileUploadModalLabel" aria-hidden="true">
    <form action="{{ route('MediaVideos.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="modal-content fileUploadModal">
                ...
                <div class="modal-body">
                    ...
                    <div class="text-center">
                        <button type="button" class="btn btn-primary br_8" id="chooseFilesButton">Choose Videos</button>
                        <input type="file" name="video[]" id="fileInput" class="d-none" multiple accept="video/mp4,video/avi,video/mkv,video/mov,video/wmv,video/flv,video/webm,video/3gp" />
                    </div>
                    ...
                    <p class="text-custom mt-2" style="font-size: 0.9rem;">
                        Note: You can upload Videos up to 15 MB each.
                    </p>
                </div>
...
<script src="{{ asset('js/Admin/media_videos.js') }}"></script>
