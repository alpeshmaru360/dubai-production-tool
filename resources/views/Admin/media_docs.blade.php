...
<link href="{{ asset('css/Admin/media_docs.css') }}" rel="stylesheet" />
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <iframe src="{{ asset('sales_manager/media_database/documents/' . $Document->name) }}" width="50%" height="150px"></iframe>

                        {{-- Preview Office files (Word, Excel, PowerPoint) --}}
                        @elseif(in_array($fileExtension, ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'ods', 'odt', 'odp']))
                        <iframe src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(asset('sales_manager/media_database/documents/' . $Document->name)) }}" width="50%" height="150px"></iframe>

                        {{-- Preview Text files --}}
                        @elseif(in_array($fileExtension, ['txt', 'csv', 'log', 'xml', 'json', 'md']))
                        <pre style="width: 100%; height: 150px; overflow: auto; border: 1px solid #ccc;">
                        {{ file_get_contents(public_path('sales_manager/media_database/documents/'. $Document->name)) }}
                        </pre>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            @if(!$isText && !$isMsOffice)
                            <a href="{{ asset('sales_manager/media_database/documents/' . $Document->name) }}" target="_blank" class="action-button" title="View">
                            ...
                            @endif
                            <a href="{{ asset('sales_manager/media_database/documents/' . $Document->name) }}" class="action-button" download title="Download">
                            ...
                            <form action="{{ route('MediaDocs.destroy', $Document->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="fileUploadModal" tabindex="-1" aria-labelledby="fileUploadModalLabel" aria-hidden="true">
    <form action="{{ route('MediaDocs.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="modal-content fileUploadModal">
                ...
                <div class="modal-body">
                    ...
                    <div class="text-center">
                        <button type="button" class="btn btn-primary br_8" id="chooseFilesButton">Choose Documents</button>
                        <input type="file" name="document[]" id="fileInput" class="d-none" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.odt,.ods" />
                    </div>
                    ...
                    <p class="text-custom mt-2" style="font-size: 0.9rem;">
                        Note: You can upload Documents up to 15 MB each.
                    </p>
                </div>
...
<script src="{{ asset('js/Admin/media_docs.js') }}"></script>
