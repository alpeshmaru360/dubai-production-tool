...
<link href="{{ asset('css/Admin/product_type_docs/booster_docs.css') }}" rel="stylesheet" />
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="document_preview">
                        ...
                        <iframe src="{{ asset('sales_manager/document_portal/product_types/booster_docs/' . $document->name) }}" width="100%" height="150px"></iframe>

                        {{-- Preview Office files (Word, Excel, PowerPoint) --}}
                        @elseif(in_array($fileExtension, ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'ods', 'odt', 'odp']))
                        <iframe src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(asset('sales_manager/document_portal/product_types/booster_docs/' . $document->name)) }}" width="100%" height="150px"></iframe>

                        {{-- Preview Image files --}}
                        @elseif(in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'tiff', 'webp']))
                        <img src="{{ asset('sales_manager/document_portal/product_types/booster_docs/' . $document->name) }}" alt="{{ $document->name }}" class="img-preview">

                        {{-- Preview Video files --}}
                        @elseif(in_array($fileExtension, ['mp4', 'webm', 'ogg', 'avi', 'mov', 'mkv', 'flv', 'wmv', '3gp']))
                        <video autoplay muted loop src="{{ asset('sales_manager/document_portal/product_types/booster_docs/' . $document->name) }}" alt="{{ $document->name }}" class="img-preview">
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <a href="{{ asset('sales_manager/document_portal/product_types/booster_docs/' . $document->name) }}" target="_blank" class="action-button" title="View">
                            ...
                            @endif
                            <a href="{{ asset('sales_manager/document_portal/product_types/booster_docs/' . $document->name) }}" class="action-button" download title="Download">
                            ...
                            <form action="{{ route('BoosterDoc.destroy', $document->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
...
<div class="modal fade" id="fileUploadModal" tabindex="-1" aria-labelledby="fileUploadModalLabel" aria-hidden="true">
    <form action="{{ route('BoosterDoc.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
...
<script src="{{ asset('js/Admin/Product_type_docs/booster_docs.js') }}"></script>
