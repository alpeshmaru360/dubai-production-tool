...
<link href="{{ asset('css/sales_manager/product_type_docs/borehole_docs.css') }}" rel="stylesheet" />   
    ...
    <div class="row g-4 d-flex justify-content-between">
        ...
        <div class="col-md-5 float-right">
            <div class="search-container mb-5">
                <form action="{{ url('sale_manager/borehole_docs') }}" method="GET" class="d-flex align-items-center">
            ...
            <div class="col-md-5 col-lg-3">
                <div class="card document-card shadow-sm border-0 text-center p-3">
                    ...
                    <div class="icon-container mb-3 mx-auto">
                        ...
                        $isImage = in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'tiff', 'webp']);
                        @endphp

                        {{-- Preview PDF file --}}
                        @if(in_array(strtolower($fileExtension), ['pdf']))
                        <iframe src="{{ asset('sales_manager/document_portal/product_types/borehole_docs/' . $document->name) }}" width="100%" height="200px"></iframe>

                        {{-- Preview Image file --}}
                        @elseif($isImage)
                        <img src="{{ asset('sales_manager/document_portal/product_types/borehole_docs/' . $document->name) }}" alt="{{ $document->name }}" class="img-preview">

                        {{-- Preview Video file --}}
                        @elseif(in_array(strtolower($fileExtension), ['mp4', 'webm', 'ogg', 'avi', 'mov', 'mkv', 'flv', 'wmv', '3gp']))
                        <video autoplay muted loop src="{{ asset('sales_manager/document_portal/product_types/borehole_docs/' . $document->name) }}" alt="{{ $document->name }}" class="img-preview">

                            {{-- Preview Office files (Word, Excel) --}}
                            @elseif(in_array(strtolower($fileExtension), ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx']))
                            <iframe src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(asset('sales_manager/document_portal/product_types/borehole_docs/' . $document->name)) }}" width="100%" height="200px"></iframe>
                            ...
                            {{ file_get_contents(public_path('sales_manager/document_portal/product_types/borehole_docs/'. $document->name)) }}
                            </pre>
                    ...
                    <div class="d-flex justify-content-center gap-3">
                        ...
                        <a href="{{ asset('sales_manager/document_portal/product_types/borehole_docs/' . $document->name) }}" class="btn btn-secondary btn-sm w-50" target="_blank">
                            <i class="bi bi-eye"></i> View
                        </a>
                        @endif
                        <a href="{{ asset('sales_manager/document_portal/product_types/borehole_docs/' . $document->name) }}" class="btn btn-primary btn-sm {{ $isImage ? 'w-100' : 'w-50' }}" download>
