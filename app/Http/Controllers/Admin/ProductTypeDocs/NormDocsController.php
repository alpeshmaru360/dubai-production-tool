...
class NormDocsController extends Controller
{
    public function index()
    {
        // Get all documents and pass them to the view
        $documents = NormDocs::orderBy('created_at', 'desc')->get();
        return view('Admin.Product_type_docs/norm_docs', compact('documents'));
    }
        ...
        try {
            ...
            if ($request->hasFile('document')) {
                foreach ($request->file('document') as $file) {
                    ...
                    $path = public_path('sales_manager/document_portal/product_types/norm_docs/' . $sanitizedFilename);

                    // Move the uploaded file to the target path
                    $file->move(public_path('sales_manager/document_portal/product_types/norm_docs/'), $sanitizedFilename);
                    ...
                    NormDocs::create([
                        ...
                        'document_path' => 'sales_manager/document_portal/product_types/norm_docs/' . $sanitizedFilename,
                    ]);
                }
            }

            return redirect()->route('NormDoc')->with('success', 'Documents uploaded successfully.');
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $document = NormDocs::findOrFail($id);

            // Define the file path
            $filePath = public_path('sales_manager/document_portal/product_types/norm_docs/' . $document->name);
