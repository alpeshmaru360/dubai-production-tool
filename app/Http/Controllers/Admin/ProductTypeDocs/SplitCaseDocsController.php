...
class SplitCaseDocsController extends Controller
{
    public function index()
    {
        // Get all documents and pass them to the view
        $documents = SplitCaseDocs::orderBy('created_at', 'desc')->get();
        return view('Admin.Product_type_docs/split_case_docs', compact('documents'));
    }
        ...
        try {
            ...
            if ($request->hasFile('document')) {
                foreach ($request->file('document') as $file) {
                    ...
                    $path = public_path('sales_manager/document_portal/product_types/split_case_docs/' . $sanitizedFilename);

                    // Move the uploaded file to the target path
                    $file->move(public_path('sales_manager/document_portal/product_types/split_case_docs/'), $sanitizedFilename);

                    // Store each document in the database with the sanitized file name
                    SplitCaseDocs::create([
                        'name' => $sanitizedFilename, // Storing the sanitized file name in the 'name' field
                        'document_path' => 'sales_manager/document_portal/product_types/split_case_docs/' . $sanitizedFilename,
                    ]);
                }
            }

            return redirect()->route('SplitCaseDoc')->with('success', 'Documents uploaded successfully.');
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $document = SplitCaseDocs::findOrFail($id);

            // Define the file path
            $filePath = public_path('sales_manager/document_portal/product_types/split_case_docs/' . $document->name);
