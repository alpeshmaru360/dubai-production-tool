...
class BoreoleDocsController extends Controller
{
    public function index()
    {
        // Get all documents and pass them to the view
        $documents = BoreoleDocs::orderBy('created_at', 'desc')->get();
        return view('Admin.Product_type_docs/borehole_docs', compact('documents'));
    }
        ...
        try {
            ...
            if ($request->hasFile('document')) {
                foreach ($request->file('document') as $file) {
                    ...
                    $path = public_path('sales_manager/document_portal/product_types/borehole_docs/' . $sanitizedFilename);

                    // Move the uploaded file to the target path
                    $file->move(public_path('sales_manager/document_portal/product_types/borehole_docs/'), $sanitizedFilename);
                    ...
                    BoreoleDocs::create([
                        ...
                        'document_path' => 'sales_manager/document_portal/product_types/borehole_docs/' . $sanitizedFilename,
                    ]);
                }
            }
            return redirect()->route('BoreholeDoc')->with('success', 'Documents uploaded successfully.');
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $document = BoreoleDocs::findOrFail($id);

            // Define the file path
            $filePath = public_path('sales_manager/document_portal/product_types/borehole_docs/' . $document->name);
