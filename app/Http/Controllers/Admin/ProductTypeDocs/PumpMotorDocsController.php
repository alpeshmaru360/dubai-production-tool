...
class PumpMotorDocsController extends Controller
{
    public function index()
    {
        // Get all documents and pass them to the view
        $documents = PumpMotorDocs::orderBy('created_at', 'desc')->get();
        return view('Admin.Product_type_docs/pump_motor_docs', compact('documents'));
    }
        ...
        try {
            ...
            if ($request->hasFile('document')) {
                foreach ($request->file('document') as $file) {
                    ...
                    $path = public_path('sales_manager/document_portal/product_types/pump_motor_docs/' . $sanitizedFilename);

                    // Move the uploaded file to the target path
                    $file->move(public_path('sales_manager/document_portal/product_types/pump_motor_docs/'), $sanitizedFilename);

                    // Store each document in the database with the sanitized file name
                    PumpMotorDocs::create([
                        'name' => $sanitizedFilename, // Storing the sanitized file name in the 'name' field
                        'document_path' => 'sales_manager/document_portal/product_types/pump_motor_docs/' . $sanitizedFilename,
                    ]);
                }
            }

            return redirect()->route('PumpMotorDoc')->with('success', 'Documents uploaded successfully.');
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $document = PumpMotorDocs::findOrFail($id);

            // Define the file path
            $filePath = public_path('sales_manager/document_portal/product_types/pump_motor_docs/' . $document->name);
