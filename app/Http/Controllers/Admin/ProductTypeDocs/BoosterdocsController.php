...
class BoosterdocsController extends Controller
{
    public function index()
    {
        // Get all documents and pass them to the view
        $documents = BoosterDocs::orderBy('created_at', 'desc')->get();
        return view('Admin.Product_type_docs/booster_docs', compact('documents'));
    }
        ...
        try {
            ...
            if ($request->hasFile('document')) {
                foreach ($request->file('document') as $file) {
                    ...
                    $path = public_path('sales_manager/document_portal/product_types/booster_docs/' . $sanitizedFilename);

                    // Move the uploaded file to the target path
                    $file->move(public_path('sales_manager/document_portal/product_types/booster_docs/'), $sanitizedFilename);
                    ...
                    BoosterDocs::create([
                        ...
                        'document_path' => 'sales_manager/document_portal/product_types/booster_docs/' . $sanitizedFilename,
                    ]);
                }
            }
            return redirect()->route('BoosterDoc')->with('success', 'Documents uploaded successfully.');
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $document = BoosterDocs::findOrFail($id);

            // Define the file path
            $filePath = public_path('sales_manager/document_portal/product_types/booster_docs/' . $document->name);
