...
class GeneralDocumentController extends Controller
{
    public function index()
    {
        ...
        $documents = GeneralDocuments::orderBy('created_at', 'desc')->get();
        return view('Admin.general_documents', compact('documents'));
    }
        ...
        try {
            ...
            if ($request->hasFile('document')) {
                foreach ($request->file('document') as $file) {
                    ...
                    $path = public_path('sales_manager/document_portal/general_documents/' . $sanitizedFilename);

                    // Move the uploaded file to the target path
                    $file->move(public_path('sales_manager/document_portal/general_documents'), $sanitizedFilename);

                    // Store each document in the database with the sanitized file name
                    GeneralDocuments::create([
                        'name' => $sanitizedFilename, // Storing the sanitized file name in the 'name' field
                        'document_path' => 'sales_manager/document_portal/general_documents/' . $sanitizedFilename,
                    ]);
                }
            }

            return redirect()->route('GenralDocument')->with('success', 'Documents uploaded successfully.');
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $document = GeneralDocuments::findOrFail($id);

            // Define the file path
            $filePath = public_path('sales_manager/document_portal/general_documents/' . $document->name);
